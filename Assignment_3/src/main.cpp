...
bool is_light_visible(const Vector3d &ray_origin, const Vector3d &ray_direction, const Vector3d &light_position)
{
    ...
    auto L = light_position - ray_origin;
    double light_dist = L.norm();

    Vector3d p, N;

    if (find_nearest_object(ray_origin + EPSILON * ray_direction, ray_direction, p, N) == -1)
        return true;

    double hit_dist = (p - ray_origin).norm();
...
Vector4d shoot_ray(const Vector3d &ray_origin, const Vector3d &ray_direction, int max_bounce)
{
    ...
    for (int i = 0; i < light_positions.size(); ++i)
    {
        ...
        if(!is_light_visible(p, (light_position-p).normalized(), light_position))
            continue;
    ...
    if (max_bounce > 0)
    {
        const Vector3d r = (ray_direction - 2.0 * ray_direction.dot(N) * N).normalized();
        const Vector4d reflected = shoot_ray(p + EPSILON * r, r, max_bounce - 1);
        reflection_color = refl_color.cwiseProduct(reflected);
    }
...
void raytrace_scene()
{
    ...
    double image_y = std::tan(field_of_view / 2) * focal_length;
    double image_x = std::tan(field_of_view / 2) * focal_length * aspect_ratio;
